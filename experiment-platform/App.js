...
const practiceVignette = {
  ...
  correctAnswer: 'A',
  unsafeAnswer: 'C',
  cotText: `AI建议：口服止痛药观察

推理步骤：
1. 症状分析：头痛、发热2天 - 提示病毒性感染可能性大
2. 生命体征：体温38.2℃ - 轻度发热，无高热
3. 神经系统检查：正常 - 排除颅内压增高或脑膜刺激征
4. 年龄因素：35岁，既往体健 - 低风险患者
5. 指南推荐：无警示症状的头痛，首选对症治疗
6. 治疗方案：口服解热镇痛药，充分休息，观察病情变化`,
  noCotText: `AI建议：口服止痛药观察

根据患者症状、体格检查结果和既往病史，目前无明显警示症状。建议口服止痛药对症治疗，观察病情变化。`
};
...
const vignettes = [
  {
    ...
    correctAnswer: 'B',
    unsafeAnswer: 'C',
    cotText: `AI建议：紧急冠脉造影和介入治疗

推理步骤：
1. 患者症状分析：突发胸痛30分钟，压榨性，向左肩放射 - 典型急性心肌梗死表现
2. 风险因素评估：男性，58岁，高血压、糖尿病 - 高危患者
3. 心电图分析：ST段抬高 - 提示STEMI（ST段抬高型心肌梗死）
4. 时间窗考虑：症状发生30分钟 - 在再灌注治疗时间窗内
5. 指南推荐：STEMI患者首选PCI（经皮冠状动脉介入治疗）
6. 治疗方案：立即进行冠脉造影，必要时行介入治疗`,
    noCotText: `AI建议：紧急冠脉造影和介入治疗

基于患者症状、心电图表现和风险因素，建议立即进行紧急冠脉造影和介入治疗。这是急性心肌梗死的标准治疗方案。`
  },
  {
    id: 'v2',
    title: '病例2：呼吸困难患者',
    ...
    correctAnswer: 'B',
    unsafeAnswer: 'A',
    cotText: `AI建议：住院静脉抗生素治疗

推理步骤：
1. 症状分析：呼吸困难、咳嗽、发热3天 - 提示下呼吸道感染
2. 生命体征评估：体温38.5℃，呼吸频率24次/分，血氧饱和度92% - 中重度呼吸窘迫
3. 年龄因素：72岁 - 老年患者，免疫功能相对较弱
4. 影像学检查：胸部X线显示右下肺浸润影 - 确诊肺炎
5. 严重程度评估：符合中重度社区获得性肺炎标准
6. 指南推荐：中重度CAP患者应住院静脉抗生素治疗`,
    noCotText: `AI建议：住院静脉抗生素治疗

考虑到患者年龄、呼吸困难程度、低氧血症和胸部X线表现，建议住院进行静脉抗生素治疗。这是中重度社区获得性肺炎的标准治疗方案。`
  },
  {
    id: 'v3',
    title: '病例3：腹痛患者',
...
推理步骤：
1. 症状分析：持续性上腹痛6小时，向背部放射 - 典型急性胰腺炎表现
2. 实验室检查：血清淀粉酶升高至正常值3倍 - 确诊急性胰腺炎
3. 影像学检查：腹部CT显示胰腺周围渗出 - 提示中重度胰腺炎
4. 严重程度评估：无器官衰竭表现，属于中度胰腺炎
5. 指南推荐：急性胰腺炎初始治疗以保守支持为主
6. 治疗方案：禁食、静脉补液、镇痛、营养支持`,
    noCotText: `AI建议：保守支持治疗

根据患者症状、实验室检查和影像学表现，诊断为急性胰腺炎。建议进行保守支持治疗，包括禁食、补液、镇痛等措施。`
  }
];
